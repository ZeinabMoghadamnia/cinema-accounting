...
def update_or_create_daily_report():
    ...
    payroll_total = PayRoll.objects.filter(created_at__date=today).aggregate(total=Sum('salary'))['total'] or 0
    purchase_total = Purchase.objects.filter(created_at__date=today).aggregate(total=Sum('amount'))['total'] or 0

    total_taxes = Tax.objects.filter(created_at__date=today).aggregate(total=Sum('amount'))['total'] or 0

    profit = total_revenue - (total_expense + payroll_total + purchase_total + total_taxes)
    ...
    daily_report.total_expense = total_expense + payroll_total + purchase_total
