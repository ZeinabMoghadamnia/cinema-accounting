...
def update_or_create_daily_report(report_date):
    ...
    purchase_total = Purchase.objects.filter(date=report_date).aggregate(total=Sum('amount'))['total'] or 0

    taxes = Tax.objects.filter(date=report_date, tax_type='cinema tax').aggregate(total=Sum('tax_percentage'))['total'] or 0
